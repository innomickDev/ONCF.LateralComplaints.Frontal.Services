...
const getSubCategories = async function(req, res){    
    ...
    let reqid = uuid()
    logHelper.LogCallStart(reqid, "GetSubCategories", "Express Js", "GetSubCategories api is started successfully")
    ...
    if(response.isSuccess){        
        ...
        logHelper.LogCallEnd(reqid,"GetSubCategories", "ExpressJs",response.error.errorCode,response.error.errorMessage)
...
const getSubCategoryByCategoryCode = async function(req, res){    
    // below reqid is callId for log function
    let reqid = uuid()
    logHelper.LogCallStart(reqid, "GetSubCategoryByCategoryCode", "Express Js", "GetSubCategoryByCategoryCode api is started successfully")
    ...
    const option = optionHelper.createOption(`${webUrl.FRONT_URL}GetListSubCategoryByCategoryCode`,extraHeader,requestBody,null)
    const response = await actionHelper.actionHandler(option,'subCategoryClientResponse')
    if(response.isSuccess){         
         // below Called for log function
        logHelper.LogCallEnd(reqid,"GetSubCategoryByCategoryCode", "ExpressJs",response.error.errorCode,response.error.errorMessage)
        customResponse.isSuccess=true;
        customResponse.data = response.data;
        customResponse.error = {error :"",errorDescription : ""}
        return res.send(customResponse);
