...
const getSubSubCategories = async function(req, res){    
    ...
    let reqid = uuid()
    logHelper.LogCallStart(reqid, "GetSubSubCategories", "Express Js", "GetSubSubCategories api is started successfully")
    ...
    if(response.isSuccess){         
        ...
        logHelper.LogCallEnd(reqid,"GetSubSubCategories", "ExpressJs",response.error.errorCode,response.error.errorMessage)
...
const getSubSubCategoryBySubCategoryCode = async function(req, res){    
    // below reqid is callId for log function
    let reqid = uuid()
    logHelper.LogCallStart(reqid, "GetSubSubCategoryBySubCategoryCode", "Express Js", "GetSubSubCategoryBySubCategoryCode api is started successfully")
    ...
    const option = optionHelper.createOption(`${webUrl.FRONT_URL}GetListSubSubCategoryBySubCategoryCode`,extraHeader,bodyData,null)
    const response = await actionHelper.actionHandler(option,'subSubCategoryClientResponse')
    if(response.isSuccess)
    {        
        // below called for log function
        logHelper.LogCallEnd(reqid,"GetSubSubCategoryBySubCategoryCode", "ExpressJs",response.error.errorCode,response.error.errorMessage)
        customResponse.isSuccess=true;
        customResponse.data = response.data;
        customResponse.error = {error :"",errorDescription : ""}
        return res.send(customResponse);
