...
setVersion = function (fileName, instanceName) {
    try {
        if (fileName && instanceName) {
            //const versionPath = CACHE_PATH + instanceName + "/versions.json"
            const versionPath = path.join(__dirname, '../', '../', 'factory/', 'cache/', instanceName, '/versions.json')
            let jsonContent = fs.readFileSync(versionPath, "utf-8") || JSON.stringify({"currentVersionName":"","versionHistory":[]}) ;            
            const versionContent = JSON.parse(jsonContent);            
            //console.log(versionContent)
            versionContent.currentVersionName = fileName;
            if ( versionContent.versionHistory && versionContent.versionHistory.length < 1) {
